    ...
    <header class="border-b border-white/[0.07] bg-black/40 backdrop-blur-xl sticky top-0 z-10">
        <div class="container mx-auto px-6 py-4 flex items-center justify-between">
            <div>
                ...
                <p class="text-white/40 text-xs mt-0.5">{blogs.length} blog posts</p>
            </div>
    ...
    <main class="container mx-auto px-6 py-10">
        {#if blogs.length === 0}
            <div class="text-center py-24">
                ...
                <a href="/admin/new" class="px-6 py-3 rounded-xl bg-[#FF014F] text-white font-bold inline-block hover:bg-[#cc0040] transition-colors">
        ...
        {:else}
            <div class="grid gap-3">
                {#each blogs as blog}
                    <div class="group flex items-center justify-between gap-4 border border-white/[0.07] bg-white/[0.03] rounded-xl px-5 py-4 hover:border-white/15 transition-all duration-200">
                        <div class="flex items-center gap-4 min-w-0">
                            <!-- Published indicator -->
                            <span class="flex-shrink-0 w-2 h-2 rounded-full {blog.published ? 'bg-green-400' : 'bg-white/20'}"></span>

                            <div class="min-w-0">
                                <h3 class="text-white font-semibold text-sm truncate">{blog.title}</h3>
                                <p class="text-white/30 text-xs mt-0.5">
                                    <span class="font-mono">/blog/{blog.slug}</span>
