...
<div class="min-h-screen bg-[#1D232A] text-white">
    <header class=" backdrop-blur-xl sticky top-0 z-10">
        ...
        >
            <h1 class="text-lg font-black truncate max-w-sm">
                Edit: {blog?.title}
            </h1>
            <a
                href="/admin"
    ...
    <main class="container mx-auto px-6 py-10">
        {#if form?.error}
            <div
                class="mb-6 px-4 py-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm"
            >
                {form.error}
            </div>
        {/if}
        ...
        >
            ...
            <div class="flex items-center gap-3">
                <input
                    ...
                    class="w-4 h-4 accent-[#FF014F]"
                />
                <label
                    for="published"
                    class="text-sm text-white/60 cursor-pointer"
                    >Published</label
                >
            </div>
            ...
            <div>
                <div class="flex gap-2 mb-3">
                    <button
                        type="button"
                        on:click={() => (activeTab = "en")}
                        class="px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors {activeTab ===
                        'en'
            ...
            <div class="flex items-center justify-between pt-2">
                <div class="flex gap-3">
                    ...
                    <a
                        href="/admin"
                        class="px-6 py-3 rounded-xl border border-white/10 text-white/60 hover:text-white hover:border-white/20 transition-colors font-semibold"
                    >
                        Cancel
                    </a>
                </div>
                ...
                {:else}
                    <div class="flex items-center gap-2">
                        ...
                        <button
                            ...
                            class="text-sm text-white/30 hover:text-white transition-colors"
                            >Cancel</button
                        >
                    </div>
                {/if}
