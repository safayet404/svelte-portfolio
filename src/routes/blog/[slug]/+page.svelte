...
<svelte:head>
    <title>{blog ? ($locale === "bn" ? blog.title_bn : blog.title) : "Blog"} — Safayet</title>
</svelte:head>
...
<div class="min-h-screen bg-[#050014]">
    {#if blog}
        ...
        <div class="relative overflow-hidden py-20 border-b border-white/[0.07]">
            ...
            <div class="container mx-auto px-6 relative z-10 max-w-3xl">
                <a href="/blog" class="inline-flex items-center gap-2 text-white/40 hover:text-white text-sm mb-8 transition-colors duration-200">
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    {#each blog.tags as tag}
                        <span class="text-xs font-semibold px-2.5 py-1 rounded-full bg-[#FF014F]/10 text-[#FF014F] border border-[#FF014F]/20">{tag}</span>
                    {/each}
                </div>
                ...
                <p class="text-white/40 mt-4 text-sm">{formatDate(blog.date)}</p>

                {#if blog.cover}
                    <div class="mt-8 rounded-xl overflow-hidden h-64 md:h-80">
                        <img src={blog.cover} alt={blog.title} class="w-full h-full object-cover object-top" />
                    </div>
                {/if}
            </div>
        </div>
        ...
        <div class="container mx-auto px-6 py-14 max-w-3xl">
            ...
            <div class="mt-14 pt-8 border-t border-white/[0.07]">
                <a href="/blog" class="inline-flex items-center gap-2 border border-white/20 text-white text-sm font-semibold px-5 py-2.5 rounded-lg hover:bg-white/5 transition-colors duration-200">
    ...
    {:else}
        <div class="container mx-auto px-6 py-32 text-center">
            <p class="text-white/40 text-lg">{$locale === "bn" ? "ব্লগটি পাওয়া যায়নি।" : "Blog post not found."}</p>
            <a href="/blog" class="mt-4 inline-block text-[#FF014F] hover:underline">{$locale === "bn" ? "সব ব্লগ দেখুন" : "View all posts"}</a>
        </div>
    {/if}
</div>
